...
## ۳. قرار دادن محتوا (خیلی مهم)

همه‌ی محتوا داخل پوشه‌ی **`mods/`** در ریشه‌ی پروژه است و به‌عنوان *resource* کنار برنامه بسته‌بندی می‌شود:

```text
mods/
├── graphics/                 ← پک گرافیکی ۵ حالته (هر پوشه آینه‌ی پوشه‌ی بازی)
│   ├── common/  (اختیاری)
│   ├── low/  medium/  high/  veryhigh/  ultra/
└── addons/                   ← افزونه‌های جانبی
    └── <id>/ { mod.json, preview.png, files/ }
```

> راهنمای کامل با مثال و قوانین: **[MODS-GUIDE.md](MODS-GUIDE.md)**
> **حجم محتوا مستقیماً به حجم exe اضافه می‌شود** (NSIS آن‌ها را با LZMA فشرده می‌کند).
...
- [ ] محتوا داخل `mods/graphics/<سطح>/` و `mods/addons/` قرار دارد
